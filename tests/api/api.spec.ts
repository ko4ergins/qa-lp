...
test.describe('@API tests, playwright/request', () => {
   ...
   test(`API-4 POST /pockemons, User can create new Pokemon`, async ({ request }) => {
      ...
      await expect.soft(json).toMatchObject(newPokemonPayload);
   ...
   test(`API-5 POST /register, User can create new account`, async ({ request }) => {
      ...
      const { ok, json, status } = await registerRequest.registerUser(newUserPayload);

      await expect.soft(ok).toBe(true);
      await expect.soft(status).toBe(200);
      await expect.soft(json.id).toBeGreaterThan(1);
      await expect.soft(json.token.length).toBeGreaterThan(1);
   });
});
